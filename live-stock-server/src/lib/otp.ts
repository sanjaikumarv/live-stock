 ...
 * @param  {boolean} options.lowerCaseAlphabets Default: `false` true value includes lowercase alphabets in OTP
 * @param  {boolean} options.upperCaseAlphabets Default: `false` true value includes uppercase alphabets in OTP
 * @param  {boolean} options.specialChars Default: `false` true value includes specialChars in OTP
 */

export default function generateOtp(length: number, options?: any) {
  length = length || 10;
